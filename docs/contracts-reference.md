...
| Stable-campaign recovery requirements | `internal/provisioning/campaignmedia` | The separate v1alpha2 requirements contract binds the staging plan to both complete v1alpha2 capture envelopes, including every GPT lineage and recovery range; all backup, approval, and physical-write readiness fields remain false |
| Stable-campaign staging sandbox | `internal/provisioning/campaignsandbox` | Synthetic previews and acknowledgements bind private regular-file attachments, contracts, backup ranges, and staged bytes; durable attempt journals block retry and completion requires independent readback |
| Candidate campaign device staging | `internal/provisioning/campaignstaging` | Per-leg durable backups, local acknowledgement, execute-once writes and reopened readback; Linux attachment checks and virtual-block tests; receipts cannot close hardware claims |
| First-baseline preparation packet | `internal/provisioning/campaignpacket` | Independently rehashes public sources, four complete planned partitions and run-1/run-2 bindings; derives remaining traces and witnesses without asserting their observation |
...
| Stable-campaign recovery | `kaiba-rpi5-stable-campaign-recovery-requirements` (descriptive v1alpha2 catalog from previously captured JSON files) |
| Stable-campaign sandbox | `kaiba-rpi5-stable-campaign-sandbox` (synthetic regular-file `prepare`, `approve`, and `execute` commands; no physical write authority) |
| Stable-campaign staging candidate | `kaiba-rpi5-stable-campaign-stage` (generic device operations fail closed; `lib.mkRpi5StableCampaignStaging` binds one reviewed leg, plan and complete payload set) |
| Stable-campaign preparation packet | `kaiba-rpi5-stable-campaign-packet` (public-byte consistency for runs 1 and 2; no hardware or claim closure) |
