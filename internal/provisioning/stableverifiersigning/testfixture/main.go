...
const (
	fixtureBackendID  = "backend:nix-stable-verifier-fixture"
	fixtureSocketPath = "/run/kaiba-stable-verifier-fixture/signing.sock"
...
func run(arguments []string) error {
	...
	loadedPlan, err := stableverifiersigning.LoadPlanDirectory(configuration.planDirectory)
	if err != nil {
		return err
	}
	intent, err := stableverifiersigning.ParseIntent(loadedPlan.ReleaseIntentJSON)
	if err != nil {
	...
	authorization, err := stableverifiersigning.NewAuthorization(
...
func parseOptions(arguments []string) (options, error) {
	flags := flag.NewFlagSet("stable-verifier-signing-testfixture", flag.ContinueOnError)
