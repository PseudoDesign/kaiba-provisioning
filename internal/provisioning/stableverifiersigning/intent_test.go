...
func TestIntentStrictCanonicalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{"duplicate", strings.Replace(valid, `"authorization_scope":"stable_campaign_verifier_boot"`, `"authorization_scope":"stable_campaign_verifier_boot","authorization_scope":"stable_campaign_verifier_boot"`, 1), "duplicated"},
...
func testIntent(t *testing.T) Intent {
	...
	intent, err := NewIntent(IntentParameters{
		...
		UnsignedManifestDigest:  testDigest("1"),
