...
const (
	fixtureBackendID  = "backend:nix-stable-offline-fixture"
	fixtureSocketPath = "/run/kaiba-stable-offline-fixture/signing.sock"
...
func run(arguments []string) error {
	...
	loadedPlan, err := nativeofflinesigning.LoadPlanDirectory(configuration.planDirectory)
	if err != nil {
		return err
	}
	intent, err := nativeofflinesigning.ParseIntent(loadedPlan.ReleaseIntentJSON)
	if err != nil {
		return fmt.Errorf("parse offline signing intent: %w", err)
	}
	...
	if !bytes.Equal(publicPEM, loadedPlan.PublicPEM) {
		return errors.New("fixture private key does not match the offline signing plan public.pem")
	}

	authorization, err := nativeofflinesigning.NewAuthorization(
...
func parseOptions(arguments []string) (options, error) {
	flags := flag.NewFlagSet("stable-offline-signing-testfixture", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	var configuration options
	flags.StringVar(&configuration.planDirectory, "plan", "", "absolute offline signing plan directory")
