...
// accompanying a offline plan/result snapshot. The caller must separately
...
func VerifyEvidence(
	...
	if bundle.Sum(publicPEM) != intent.PublicKeyFileDigest {
		return fmt.Errorf("public.pem digest does not match the offline signing intent")
	}
	...
	if fingerprint != intent.PublicKeyFingerprint {
		return fmt.Errorf("public.pem fingerprint does not match the offline signing intent")
	}
	...
	if result.PlanID != plan.PlanID || result.PlanDigest != planDigest ||
		...
		return fmt.Errorf("signing result does not match the offline signing plan")
	}
	...
	if err := (Authorization{Approval: approval, Registry: registry}).Validate(intent); err != nil {
		return fmt.Errorf("offline signing authorization: %w", err)
	}
...
// Finalize verifies the offline-only plan, boot signature/result and exact
