// Package nativeofflinesigning defines the narrow, public authorization
// contract used to sign exactly one offline Raspberry Pi 5
// boot image.
//
...
const (
	IntentSchemaV1Alpha1 = "kaiba.provisioning.rpi5-native-offline-signing-intent/v1alpha1"
	AuthorizationScope   = "native_offline_boot"
	MaxIntentBytes       = 64 * 1024

	intentDigestDomain     = "kaiba.provisioning.rpi5-native-offline-signing-intent.v1alpha1"
...
// constructs the one-input offline signing intent. Versioned constants and the
...
// Intent binds the exact unsigned offline manifest, the reviewed public key,
// and the only byte string authorized for a private-key operation. It is intentionally distinct from the generic
...
// ParseIntent strictly decodes and validates one bounded offline signing
...
func ParseIntent(data []byte) (Intent, error) {
	if len(data) == 0 || len(data) > MaxIntentBytes {
		return Intent{}, fmt.Errorf("offline signing intent size must be between 1 and %d bytes", MaxIntentBytes)
	}
	var intent Intent
	if err := strictDecode(data, &intent); err != nil {
		return Intent{}, fmt.Errorf("decode offline signing intent: %w", err)
	}
	if err := intent.Validate(); err != nil {
		return Intent{}, err
	}
	...
	if !bytes.Equal(data, canonical) {
		return Intent{}, errors.New("offline signing intent must use its exact canonical JSON representation")
	}
	return intent, nil
}
...
func (intent Intent) Validate() error {
	if intent.SchemaVersion != IntentSchemaV1Alpha1 {
		return fmt.Errorf("unsupported offline signing intent schema_version %q", intent.SchemaVersion)
	}
...
func (intent Intent) CanonicalJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encode canonical offline signing intent: %w", err)
	}
	if len(encoded) > MaxIntentBytes {
		return nil, fmt.Errorf("canonical offline signing intent exceeds %d bytes", MaxIntentBytes)
	}
	return encoded, nil
}
