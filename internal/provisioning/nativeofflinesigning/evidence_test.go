...
// manifest tests and Nix integration exercise actual offline artifact shape.
...
func newEvidenceFixtureWithTimes(t *testing.T, signedAt, intentAt string) evidenceFixture {
	...
	boot := []byte("one synthetic offline boot image")
	intent := testIntent(t)
	...
	plan := signedboot.Plan{SchemaVersion: signedboot.PlanSchemaV1Alpha2, PlanID: "offline:synthetic", ReleaseIntentDigest: intentDigest, BootImageDigest: intent.SigningInput.Digest, BootImageSizeBytes: intent.SigningInput.SizeBytes, PublicKeyFingerprint: fingerprint, SignerPolicyDigest: policyDigest, SourceDateEpoch: intent.SourceDateEpoch}
	...
	err = signedboot.SignWithLoader(context.Background(), planPath, signedPath, signedboot.SignConfig{
		...
		RequestSignature: func(_ context.Context, socket string, artifact []byte) (signinggate.Result, error) {
			...
			receipt := signinggate.Receipt{SchemaVersion: signinggate.ReceiptSchemaV1Alpha3, Grant: grant, RequestDigest: requestDigest, BackendID: "backend:synthetic-offline", SignatureHex: hex.EncodeToString(signature), SignatureDigest: bundle.Sum(signature), SignedAt: signedAt}
...
func TestFinalizeAuthenticatesExactNativeOfflineEvidence(t *testing.T) {
...
func TestVerifyEvidenceRejectsForgedMissingAndUnselectedEvidence(t *testing.T) {
	...
	for name, mutate := range map[string]func(*signedboot.Result, *[]byte, *[]byte, *[]byte){
		...
		t.Run(name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatal("accepted substituted or unauthenticated offline evidence")
			}
