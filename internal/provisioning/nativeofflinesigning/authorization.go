...
const (
	ApprovalSchemaV1Alpha1  = "kaiba.provisioning.rpi5-native-offline-signing-approval/v1alpha1"
	DecisionApproved        = "approved"
	MaxApprovalBytes        = 64 * 1024
	MaximumApprovalLifetime = 24 * time.Hour

	approvalDigestDomain = "kaiba.provisioning.rpi5-native-offline-signing-approval.v1alpha1"
)
...
func NewAuthorization(intent Intent, reviewerID, approvedAt, expiresAt string) (Authorization, error) {
	if err := intent.Validate(); err != nil {
		return Authorization{}, fmt.Errorf("offline signing intent: %w", err)
	}
...
func (authorization Authorization) Validate(intent Intent) error {
	if err := intent.Validate(); err != nil {
		return fmt.Errorf("offline signing intent: %w", err)
	}
	...
	if approval.SourceRevision != intent.SourceRevision || approval.SigningIntentDigest != intentDigest {
		return errors.New("approval does not identify the supplied offline signing intent")
	}
	if approval.SigningInput != intent.SigningInput {
		return errors.New("approval signing input does not exactly match the offline signing intent")
	}
	approvedAt, _ := parseCanonicalTime(approval.ApprovedAt, "approved_at")
	if approvedAt.Unix() < int64(intent.SourceDateEpoch) {
		return errors.New("approved_at precedes the offline signing intent source_date_epoch")
	}
...
func (approval Approval) CanonicalJSON() ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("encode canonical offline signing approval: %w", err)
	}
	if len(encoded) > MaxApprovalBytes {
		return nil, fmt.Errorf("canonical offline signing approval exceeds %d bytes", MaxApprovalBytes)
	}
	return encoded, nil
}
...
func ParseApproval(data []byte) (Approval, error) {
	payload, err := canonicalPayload(data, MaxApprovalBytes, "offline signing approval")
	if err != nil {
		return Approval{}, err
	}
	var approval Approval
	if err := strictDecode(payload, &approval); err != nil {
		return Approval{}, fmt.Errorf("decode offline signing approval: %w", err)
	}
	...
	if !bytes.Equal(payload, canonical) {
		return Approval{}, errors.New("offline signing approval must use its exact canonical JSON representation")
	}
	return approval, nil
}
...
func (approval Approval) validateMaterial() error {
	if approval.SchemaVersion != ApprovalSchemaV1Alpha1 {
		return fmt.Errorf("unsupported offline signing approval schema_version %q", approval.SchemaVersion)
	}
...
func (approval Approval) calculateDigest() (bundle.Digest, error) {
	...
	if err != nil {
		return "", fmt.Errorf("encode offline signing approval digest material: %w", err)
	}
	hash := sha256.New()
