...
const (
	testPhysicalEndDiskGUID = "8fa63718-33ed-4c30-81a7-3f58ce9a32df"
	testPhysicalEndBootGUID = "3b05be13-bfe3-4fd6-9390-e3463a3ce32b"
...
func installDistinctPhysicalEndGPTV1Alpha2(t *testing.T, fixture *initialGPTFixture) initialGPTV1Alpha2PhysicalEndFixture {
	t.Helper()
	...
	result := initialGPTV1Alpha2PhysicalEndFixture{
		...
		partitionGUIDs: [3]string{testPhysicalEndBootGUID, testPhysicalEndRootGUID, testPhysicalEndHashGUID},
	}
...
func TestInitialGPTRecoveryV1Alpha2CapturesDistinctValidPhysicalEndBackupLineage(t *testing.T) {
	fixture := newInitialGPTFixture(t)
	physical := installDistinctPhysicalEndGPTV1Alpha2(t, fixture)
	...
	if string(tail.Completeness) != "valid-standalone-backup-copy" ||
		string(tail.Relationship) != "same-partition-layout-distinct-disk-and-partition-guids" ||
	...
	for index, guid := range physical.partitionGUIDs {
		...
		if selected.EntryNumber != physicalPartition.EntryNumber || selected.TypeGUID != physicalPartition.TypeGUID ||
			selected.FirstLBA != physicalPartition.FirstLBA || selected.LastLBA != physicalPartition.LastLBA ||
			selected.Attributes != physicalPartition.Attributes || selected.Name != physicalPartition.Name ||
			selected.UniqueGUID == physicalPartition.UniqueGUID {
			t.Fatalf("partition %d is not the same layout with a distinct unique GUID: selected=%#v physical=%#v", index+1, selected, physicalPartition)
		}
	}
...
func TestInitialGPTRecoveryV1Alpha2RejectsMalformedOrNonDistinctPhysicalEndLineage(t *testing.T) {
	tests := map[string]func(*testing.T, *initialGPTFixture, *initialGPTV1Alpha2PhysicalEndFixture){
		...
		"reused selected partition GUID": func(t *testing.T, fixture *initialGPTFixture, physical *initialGPTV1Alpha2PhysicalEndFixture) {
...
func TestInitialGPTRecoveryV1Alpha2CanonicalRoundTripAndVersionIsolation(t *testing.T) {
	fixture := newInitialGPTFixture(t)
	installDistinctPhysicalEndGPTV1Alpha2(t, fixture)
	...
	tamperedRelationship := bytes.Replace(
		encodedV2,
		[]byte("same-partition-layout-distinct-disk-and-partition-guids"),
		[]byte("same-partition-layout-distinct-disk-and-partition-GUIDs"),
		1,
	)
	if bytes.Equal(tamperedRelationship, encodedV2) || !strings.Contains(string(tamperedRelationship), "partition-GUIDs") {
