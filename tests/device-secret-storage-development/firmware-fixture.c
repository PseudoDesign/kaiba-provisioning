/* Synthetic transport only; asserts the complete storage helper operation set. */
#include "harness.h"
#include <errno.h>
...
int test_exchange(void *arena, size_t size) {
    ...
    case 0x38090:
        ...
        v[0] = 0; return 0;
    case 0x30092: {
        if (++hmac_calls > 2 || locks != (DEVICE_TYPE | EARLY_LOCKS) || cap != 2060 ||
            v[0] || v[1] != 1 || !v[2] || v[2] > 2048) abort();
        if (fault("hmac-einval")) { errno = EINVAL; return -1; }
    ...
    default: abort(); /* No private read, signing, generation, usage write or diagnostic retry. */
    }
}
