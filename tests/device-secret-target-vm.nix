...
let
  ...
  vm = pkgs.testers.runNixOSTest {
    ...
    testScript = ''
      ...
      machine.succeed("printf 'label: gpt\\nstart=2048,size=133120,type=linux,uuid=${fixtureConfig.partition_uuid}\\n' | sfdisk /dev/vdb")
      prepare()
      machine.succeed("systemctl start device-secret-fixture.service")
      boot1 = records("create")
      machine.succeed("cp /dev/vdb1 /var/lib/device-secret-created.img")
      ...
      machine.succeed("sha256sum /dev/vdb1 > /tmp/after-create.sha256")
      ...
      machine.wait_for_unit("multi-user.target")
      prepare()
      machine.succeed("sha256sum /dev/vdb1 > /tmp/completed.sha256")
      machine.fail("systemctl start device-secret-fixture.service")
      ...
      for fault, expected in [("other-board", "volume_reopened"), ("private-returned", "raw_read_blocked"),
                              ("raw-ioctl-invalid", "raw_read_blocked"),
                              ("legacy-zeros", "legacy_read_blocked"), ("clearable-locks", "locks_cannot_clear")]:
          machine.succeed("dd if=/var/lib/device-secret-created.img of=/dev/vdb1 bs=1M conv=fsync status=none")
          ...
          machine.fail("systemctl start device-secret-fixture.service")
          output = machine.succeed("cat /run/target-events")
          errors = machine.succeed("cat /run/target-errors")
      ...
      machine.succeed("python3 -c \"import os; f=os.open('/dev/vdb1', os.O_RDWR); os.pwrite(f, b'X', 0); os.fsync(f); os.close(f)\"")
      machine.succeed("sha256sum /dev/vdb1 > /run/corrupt-journal.sha256")
      machine.fail("systemctl start device-secret-fixture.service")
...
in
# Native ARM CI has no KVM. The standard VM launcher uses its TCG fallback.
