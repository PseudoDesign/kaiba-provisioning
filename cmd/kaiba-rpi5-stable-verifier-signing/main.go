...
func productionDependencies() dependencies {
	return dependencies{
		now: time.Now, loadPlan: stableverifiersigning.LoadPlanDirectory,
...
func authorCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	planPath := flags.String("plan", "", "absolute verifier signing plan directory")
	...
	if err := requireAbsoluteClean(*outputPath, "output"); err != nil {
		return commandError(stderr, exitInvalid, "author verifier signing approval", err)
	}
	...
	authorization, err := stableverifiersigning.NewAuthorization(intent, *reviewerID, *approvedAt, *expiresAt)
	if err != nil {
		return commandError(stderr, exitInvalid, "author verifier signing approval", err)
	}
	if deps.now == nil {
		return commandError(stderr, exitInternal, "author verifier signing approval", errors.New("authoring clock is unavailable"))
	}
	...
	if err != nil {
		return commandError(stderr, exitInternal, "author verifier signing approval", err)
	}
...
func validatePlanCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	loaded, err := deps.loadPlan(*planPath)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate verifier signing plan", err)
	}
	return encodeResult(stdout, stderr, map[string]any{
...
func validateUnsignedCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	planPath := flags.String("plan", "", "absolute verifier signing plan directory")
	manifestPath := flags.String("manifest", "", "absolute unsigned verifier manifest JSON path")
	if status, done := parseFlags(flags, arguments); done {
		return status
	}
	...
	loaded, err := deps.loadPlan(*planPath)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned verifier manifest", err)
	}
	intent, err := stableverifiersigning.ParseIntent(loaded.ReleaseIntentJSON)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned verifier manifest", err)
	}
	manifest, err := readCanonicalFile(*manifestPath, stableverifiersigning.MaxUnsignedManifestBytes, "unsigned verifier manifest", deps)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned verifier manifest", err)
	}
	if err := stableverifiersigning.ValidateUnsignedManifest(manifest, intent); err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned verifier manifest", err)
	}
	return encodeResult(stdout, stderr, map[string]any{
...
func validateAuthorizationCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	planPath := flags.String("plan", "", "absolute verifier signing plan directory")
	approvalPath := flags.String("approval", "", "absolute canonical verifier approval JSON path")
	...
	loaded, err := deps.loadPlan(*planPath)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate verifier signing authorization", err)
	}
	intent, err := stableverifiersigning.ParseIntent(loaded.ReleaseIntentJSON)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate verifier signing authorization", err)
	}
	approvalData, err := readCanonicalFile(*approvalPath, stableverifiersigning.MaxApprovalBytes+1, "approval", deps)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate verifier signing authorization", err)
	}
...
func signCommand(ctx context.Context, arguments []string, stderr io.Writer, deps dependencies) int {
	...
	if deps.sign == nil {
		return commandError(stderr, exitInternal, "sign verifier boot", errors.New("signing adapter configuration is unavailable"))
	}
	if err := deps.sign(ctx, *planPath, *outputPath); err != nil {
		return commandError(stderr, exitInternal, "sign verifier boot", err)
	}
	return exitOK
}
...
func finalizeCommand(arguments []string, stderr io.Writer, deps dependencies) int {
	...
	planPath := flags.String("plan", "", "absolute verifier signing plan directory")
	signedPath := flags.String("signed", "", "absolute verifier signing-result directory")
	approvalPath := flags.String("approval", "", "absolute canonical verifier approval JSON path")
	...
	if deps.finalize == nil {
		return commandError(stderr, exitInternal, "finalize verifier boot", errors.New("finalizer configuration is unavailable"))
	}
	if err := deps.finalize(
		...
		*outputPath,
		...
		return commandError(stderr, exitInternal, "finalize verifier boot", err)
	}
	return exitOK
}
...
func printUsage(output io.Writer) {
	fmt.Fprintln(output, "usage: kaiba-rpi5-stable-verifier-signing author --plan ABSOLUTE_PLAN_DIR --reviewer-id IDENTIFIER --approved-at UTC_RFC3339_SECONDS --expires-at UTC_RFC3339_SECONDS --output ABSOLUTE_NEW_DIRECTORY")
	fmt.Fprintln(output, "       kaiba-rpi5-stable-verifier-signing validate-plan --plan ABSOLUTE_PLAN_DIR")
	fmt.Fprintln(output, "       kaiba-rpi5-stable-verifier-signing validate-unsigned --plan ABSOLUTE_PLAN_DIR --manifest ABSOLUTE_PATH")
	fmt.Fprintln(output, "       kaiba-rpi5-stable-verifier-signing validate-authorization --plan ABSOLUTE_PLAN_DIR --approval ABSOLUTE_PATH --registry ABSOLUTE_PATH")
	fmt.Fprintln(output, "       kaiba-rpi5-stable-verifier-signing sign --plan ABSOLUTE_PLAN_DIR --output ABSOLUTE_NEW_DIRECTORY")
	fmt.Fprintln(output, "       kaiba-rpi5-stable-verifier-signing finalize --plan ABSOLUTE_PLAN_DIR --signed ABSOLUTE_SIGNED_DIR --approval ABSOLUTE_PATH --registry ABSOLUTE_PATH --receipt-export ABSOLUTE_PATH --output ABSOLUTE_NEW_DIRECTORY")
}
