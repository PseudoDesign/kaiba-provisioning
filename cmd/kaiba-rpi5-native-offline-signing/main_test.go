...
func TestNativeOfflineCommandRejectsProvisionerIntentBeforeAuthoring(t *testing.T) {
...
func TestValidateAuthorizationRejectsBroadenedRegistry(t *testing.T) {
	loaded, intent := testLoadedPlan(t)
	authorization, err := nativeofflinesigning.NewAuthorization(intent, "reviewer:alice", "2026-08-18T12:00:00Z", "2026-08-18T16:00:00Z")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSigningAuthorityHasNoRuntimeOverrides(t *testing.T) {
	for _, option := range []string{
		...
		t.Run(option, func(t *testing.T) {
			...
			status := run(context.Background(), []string{
				"sign", "--plan", "/tmp/offline-plan", "--output", "/tmp/signed-offline",
...
func TestValidateUnsignedBindsNativeOfflineProfileAndExactManifestBytes(t *testing.T) {
	for _, test := range []struct {
		name       string
		...
		{"exact offline manifest", func(map[string]any) {}, true, exitOK},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			manifest := map[string]any{
				"schema_version":            nativeofflinesigning.UnsignedManifestSchemaV1Alpha1,
				"source_revision":           intent.SourceRevision,
				"unsigned_artifacts_digest": intent.UnsignedManifestDigest,
				"review_digest":             intent.UnsignedManifestDigest,
				"boot_image":                nativeofflinesigning.InputFile{Digest: intent.SigningInput.Digest, SizeBytes: intent.SigningInput.SizeBytes},
				"root_data":                 nativeofflinesigning.InputFile{Digest: intent.UnsignedManifestDigest, SizeBytes: 4096},
				"root_hash_tree":            nativeofflinesigning.InputFile{Digest: intent.UnsignedManifestDigest, SizeBytes: 4096},
				"root_integrity_digest":     intent.UnsignedManifestDigest,
				"root_data_partuuid":        nativeofflinesigning.RootDataPARTUUID,
				"root_hash_partuuid":        nativeofflinesigning.RootHashPARTUUID,
			...
			intent.UnsignedManifestDigest = bundle.Sum(append(encoded, '\n'))
			test.alter(manifest)
			encoded, err = json.Marshal(manifest)
			if err != nil {
				t.Fatal(err)
			...
			var stdout, stderr bytes.Buffer
			status := run(context.Background(), []string{
				"validate-unsigned", "--plan", "/tmp/offline-plan", "--manifest", manifestPath,
			}, &stdout, &stderr, testDependencies(loaded))
...
func TestAuthorAndValidateAuthorizationUseExactNativeOfflinePlan(t *testing.T) {
	loaded, intent := testLoadedPlan(t)
...
func testLoadedPlan(t *testing.T) (signedboot.LoadedPlan, nativeofflinesigning.Intent) {
	t.Helper()
	digest := func(character string) bundle.Digest {
		return bundle.Digest("sha256:" + strings.Repeat(character, 64))
	}
	intent, err := nativeofflinesigning.NewIntent(nativeofflinesigning.IntentParameters{
	...
	plan := signedboot.Plan{
		SchemaVersion: signedboot.PlanSchemaV1Alpha2, PlanID: "stable:offline:boot",
