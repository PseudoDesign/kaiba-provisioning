...
import (
	...
	"github.com/ams-tech/nixos-kaiba-network/provisioning/internal/provisioning/nativeofflinesigning"
...
func productionDependencies() dependencies {
	return dependencies{
		now: time.Now, loadPlan: nativeofflinesigning.LoadPlanDirectory,
...
func productionSign(ctx context.Context, planDirectory, outputDirectory string) error {
	...
	}, nativeofflinesigning.LoadPlanDirectory)
}

func productionFinalize(
	planDirectory, signedDirectory, approvalPath, registryPath, receiptExportPath, outputDirectory string,
) error {
	return nativeofflinesigning.Finalize(
		planDirectory, signedDirectory, approvalPath, registryPath, receiptExportPath, outputDirectory,
	)
}
...
func run(ctx context.Context, arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	case "validate-unsigned":
		return validateUnsignedCommand(arguments[1:], stdout, stderr, deps)
...
func authorCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	planPath := flags.String("plan", "", "absolute offline signing plan directory")
	...
	if err := requireAbsoluteClean(*outputPath, "output"); err != nil {
		return commandError(stderr, exitInvalid, "author offline signing approval", err)
	}
	loaded, err := deps.loadPlan(*planPath)
	if err != nil {
		return commandError(stderr, exitInvalid, "author offline signing approval", err)
	}
	intent, err := nativeofflinesigning.ParseIntent(loaded.ReleaseIntentJSON)
	if err != nil {
		return commandError(stderr, exitInvalid, "author offline signing approval", err)
	}
	authorization, err := nativeofflinesigning.NewAuthorization(intent, *reviewerID, *approvedAt, *expiresAt)
	if err != nil {
		return commandError(stderr, exitInvalid, "author offline signing approval", err)
	}
	if deps.now == nil {
		return commandError(stderr, exitInternal, "author offline signing approval", errors.New("authoring clock is unavailable"))
	}
	...
	if err != nil {
		return commandError(stderr, exitInternal, "author offline signing approval", err)
	}
...
func validatePlanCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	loaded, err := deps.loadPlan(*planPath)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate offline signing plan", err)
	}
	return encodeResult(stdout, stderr, map[string]any{
...
func validateUnsignedCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	flags := newFlagSet("validate-unsigned", stderr)
	planPath := flags.String("plan", "", "absolute offline signing plan directory")
	manifestPath := flags.String("manifest", "", "absolute unsigned offline manifest JSON path")
	if status, done := parseFlags(flags, arguments); done {
		return status
	}
	if flags.NArg() != 0 || *planPath == "" || *manifestPath == "" {
		flags.Usage()
		return exitUsage
	}
	loaded, err := deps.loadPlan(*planPath)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned offline manifest", err)
	}
	intent, err := nativeofflinesigning.ParseIntent(loaded.ReleaseIntentJSON)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned offline manifest", err)
	}
	manifest, err := readCanonicalFile(*manifestPath, nativeofflinesigning.MaxUnsignedManifestBytes, "unsigned offline manifest", deps)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned offline manifest", err)
	}
	if err := nativeofflinesigning.ValidateUnsignedManifest(manifest, intent); err != nil {
		return commandError(stderr, exitInvalid, "validate unsigned offline manifest", err)
	}
	return encodeResult(stdout, stderr, map[string]any{
		"status": "valid", "unsigned_manifest_digest": intent.UnsignedManifestDigest,
		"signing_intent_digest": loaded.Plan.ReleaseIntentDigest,
		"boot_image_digest":     intent.SigningInput.Digest,
	})
}
...
func validateAuthorizationCommand(arguments []string, stdout, stderr io.Writer, deps dependencies) int {
	...
	planPath := flags.String("plan", "", "absolute offline signing plan directory")
	approvalPath := flags.String("approval", "", "absolute canonical offline approval JSON path")
	...
	loaded, err := deps.loadPlan(*planPath)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate offline signing authorization", err)
	}
	intent, err := nativeofflinesigning.ParseIntent(loaded.ReleaseIntentJSON)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate offline signing authorization", err)
	}
	approvalData, err := readCanonicalFile(*approvalPath, nativeofflinesigning.MaxApprovalBytes+1, "approval", deps)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate offline signing authorization", err)
	}
	approval, err := nativeofflinesigning.ParseApproval(approvalData)
	if err != nil {
		return commandError(stderr, exitInvalid, "validate offline signing authorization", err)
	}
	...
	if err := (nativeofflinesigning.Authorization{Approval: approval, Registry: registry}).Validate(intent); err != nil {
		return commandError(stderr, exitInvalid, "validate offline signing authorization", err)
	}
	return encodeResult(stdout, stderr, map[string]any{
...
func signCommand(ctx context.Context, arguments []string, stderr io.Writer, deps dependencies) int {
	...
	if deps.sign == nil {
		return commandError(stderr, exitInternal, "sign offline boot", errors.New("signing adapter configuration is unavailable"))
	}
	if err := deps.sign(ctx, *planPath, *outputPath); err != nil {
		return commandError(stderr, exitInternal, "sign offline boot", err)
	}
	return exitOK
}
...
func finalizeCommand(arguments []string, stderr io.Writer, deps dependencies) int {
	...
	planPath := flags.String("plan", "", "absolute offline signing plan directory")
	signedPath := flags.String("signed", "", "absolute offline signing-result directory")
	approvalPath := flags.String("approval", "", "absolute canonical offline approval JSON path")
	...
	if deps.finalize == nil {
		return commandError(stderr, exitInternal, "finalize offline boot", errors.New("finalizer configuration is unavailable"))
	}
	if err := deps.finalize(
		...
		*outputPath,
		...
		return commandError(stderr, exitInternal, "finalize offline boot", err)
	}
	return exitOK
}
...
func printUsage(output io.Writer) {
	fmt.Fprintln(output, "usage: kaiba-rpi5-native-offline-signing author --plan ABSOLUTE_PLAN_DIR --reviewer-id IDENTIFIER --approved-at UTC_RFC3339_SECONDS --expires-at UTC_RFC3339_SECONDS --output ABSOLUTE_NEW_DIRECTORY")
	fmt.Fprintln(output, "       kaiba-rpi5-native-offline-signing validate-plan --plan ABSOLUTE_PLAN_DIR")
	fmt.Fprintln(output, "       kaiba-rpi5-native-offline-signing validate-unsigned --plan ABSOLUTE_PLAN_DIR --manifest ABSOLUTE_PATH")
	fmt.Fprintln(output, "       kaiba-rpi5-native-offline-signing validate-authorization --plan ABSOLUTE_PLAN_DIR --approval ABSOLUTE_PATH --registry ABSOLUTE_PATH")
	fmt.Fprintln(output, "       kaiba-rpi5-native-offline-signing sign --plan ABSOLUTE_PLAN_DIR --output ABSOLUTE_NEW_DIRECTORY")
	fmt.Fprintln(output, "       kaiba-rpi5-native-offline-signing finalize --plan ABSOLUTE_PLAN_DIR --signed ABSOLUTE_SIGNED_DIR --approval ABSOLUTE_PATH --registry ABSOLUTE_PATH --receipt-export ABSOLUTE_PATH --output ABSOLUTE_NEW_DIRECTORY")
}
