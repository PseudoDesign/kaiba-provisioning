...
import (
	...
	"github.com/ams-tech/nixos-kaiba-network/provisioning/internal/provisioning/pilotexport"
	"log"
	"net/http"
	"os"
	"time"
...
func main() {
	config := flag.String("config", "", "reviewed pilot observation configuration")
	...
	var c pilotexport.Config
	if e = handoff.Decode(b, &c); e != nil {
		log.Fatal(e)
	}
	s, e := pilotexport.New(c, *state)
	if e != nil {
		log.Fatal(e)
	}
	defer s.Close()
