...
let
  ...
  stableCampaignGPTInspector = pkgs.buildGoModule {
    ...
    passthru.kaibaRpi5StableCampaignGPTInspect = {
      ...
      physicalEndGPTV1Alpha2 = "explicit-strict-parse-and-hash-valid-distinct-backup-lineage";
      privateKeyOperations = false;
      privateKeySemanticUse = false;
      privateMaterialAbsenceProven = false;
      productionReady = false;
  ...
  pilotDevice = pkgs.buildGoModule {
    pname = "kaiba-pilot-device";
    inherit version;
    src = scopedSource "pilotDevice";
    subPackages = [ "cmd/kaiba-pilot-device" ];
    vendorHash = null;
    env.CGO_ENABLED = 0;
    doCheck = false;
    meta.mainProgram = "kaiba-pilot-device";
  };
